...
// one period of sine wave sampled at 256 equally-spaced points
...
// get the setpoint as generated by the wave/ramp generator
static void getsetpoint(int pn) {
  ...
  if(u>=p->spwaveperiod) u=0;                      // wraparound at end of period
  p->spwavephaseacc=u;                             // update phase accumulator
  u/=p->spwaveperiod;                              // now 0..1 over each period
  u+=p->spwavephase;                               // add phase and reduce to 0..1 if necessary
  if(u>=1) u-=1;
  switch(p->spwaveshape) {
default:                                           // generate the requested waveform
case WAVE_SQUARE:
...
DEB_PID { o1ch('P'); o1hex(pn); ostr(": pv="); ostr(sfloat(p->pid_pv)); ostr(" sp="); ostr(sfloat(p->setpoint)); }
  err=p->pid_pv-p->setpoint;                       // error term
  if(ABS(err)<p->deadzone) err=0;                  // in dead zone? set error to 0
  derr=(err-p->pid_perr)/(PWM_UPDATE/1000.0);      // derivative error
  p->pid_perr=err;                                 // proportional error
  p->pid_ierr+=err*(PWM_UPDATE/1000.0);            // update integral error
  CLAMP(p->pid_ierr,-p->windup,p->windup);         // clamp integral error to windup limit
DEB_PID { ostr(": e="); ostr(sfloat(err)); ostr(" ie="); ostr(sfloat(p->pid_ierr)); ostr(" de="); ostr(sfloat(derr)); }
  u=-(err*p->Kp+p->pid_ierr*p->Ki+derr*p->Kd);     // combine error terms with coefficients
DEB_PID { ostr(" cv="); ostr(sfloat(u)); onl(); }
  port_set_pwm(pn,u);                              // and set the PWM value
  return;
  }
